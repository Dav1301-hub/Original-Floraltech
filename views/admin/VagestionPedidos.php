        ...
        <div class="card-body p-0">
            <?php if (!empty($pedidos)): ?>
                <div class="table-responsive">
                    <table class="table table-sm table-striped align-middle mb-0" id="tablaPedidos">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="formEditarPedido">
                <div class="modal-header">
                    <h5 class="modal-title">Editar pedido <span id="modalEditarNumeroPedido"></span></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="id_pedido" id="editIdPedido">
                    <div class="mb-3">
                        <label class="form-label">Direccion de entrega</label>
                        <input type="text" class="form-control" name="direccion_entrega" id="editDireccion">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Fecha de entrega</label>
                        <input type="date" class="form-control" name="fecha_entrega_solicitada" id="editFechaEntrega">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="empleado_id" id="editEmpleado"></select>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Notas</label>
                        <textarea class="form-control" name="notas" id="editNotas" rows="3"></textarea>
                    </div>
...
function guardarNuevoPedido(event) {
    ...
    .then(data => {
        if (data.success) {
            alerta.innerHTML = '<div class="alert alert-success">Pedido creado correctamente</div>';
            setTimeout(() => window.location.reload(), 1000);
        } else {
            alerta.innerHTML = '<div class="alert alert-danger">' + (data.mensaje || 'No se pudo crear el pedido') + '</div>';
            btn.disabled = false;
...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('formEditarPedido')?.addEventListener('submit', guardarEdicionPedido);
    ...
    document.getElementById('nuevoPedidoModal')?.addEventListener('show.bs.modal', () => {
        cargarClientes('nuevoCliente');
    });
});
</script>
