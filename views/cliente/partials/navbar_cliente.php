...
try {
    ...
    if (!empty($user['email'])) {
        $stmt_cli = $conexion->prepare("SELECT idcli, avatar, avatar_data FROM cli WHERE email = ? LIMIT 1");
        $stmt_cli->execute([$user['email']]);
        $row = $stmt_cli->fetch(PDO::FETCH_ASSOC);
        if ($row) {
            if (!empty($row['avatar_data'])) {
                $avatar_cliente = 'avatar.php?tipo=cli&id=' . (int)$row['idcli'];
            } elseif (!empty($row['avatar']) && file_exists(__DIR__ . '/../../../' . $row['avatar'])) {
                $avatar_cliente = $row['avatar'];
            }
        }
    }
} catch (Exception $e) {
...
$navbar_volver_url = $navbar_volver_url ?? '';
?>
<nav class="navbar">
    <div class="container-fluid">
        <div class="navbar-brand navbar-brand-logo">
            ...
            <div class="navbar-brand-text">
                <span class="navbar-eyebrow">Panel Cliente</span>
        ...
        <div class="navbar-user">
            <div class="navbar-user-block">
                <span class="navbar-user-avatar-wrap">
                    <?php if (!empty($avatar_cliente)): ?>
                        <img src="<?= htmlspecialchars($avatar_cliente) ?>?v=<?= time() ?>" alt="" class="navbar-user-avatar">
                    <?php else: ?>
